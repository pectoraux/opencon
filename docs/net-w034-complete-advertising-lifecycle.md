...
| CI push + pull_request | GitHub Actions | **PASS** — remediation head `7d2fe7e`: push run `33647903435` + pull_request run `33647909652`, each with BOTH jobs success on the FIRST attempt (verify: typecheck + arch + authority + the full unit suite; integration: the real PostgreSQL + Redis service containers) — 4/4 check-runs success. The §6 rows above remain the first-submission record. | PR #70 |
